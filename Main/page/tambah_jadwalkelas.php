...
<?php
$carikode = mysqli_query($conn, "SELECT MAX(id_jadwalk) FROM jadwalkelas") or die (mysqli_error($conn));
...
if (isset($_POST['tambah'])) {
    ...
    $insertjadwal = mysqli_query($conn, "INSERT INTO jadwalkelas values ('$id_jadwalk','$id_kelas','$thn_ajaran','$semester')");
   
    if (!$insertjadwal) {
        echo "Gagal insert tabel jadwalkelas: " . mysqli_error($conn);
        die;
    }

    $allSuccess = true;
    for ($i = 0; $i < count($Kd_mapel); $i++) {
        $insert = mysqli_query($conn, "INSERT INTO detailjadwalkelas (id_jadwalk, Kd_mapel, nm_guru, hari, jam) VALUES ('$id_jadwalk', '{$Kd_mapel[$i]}', '{$nm_guru[$i]}', '{$hari[$i]}', '{$jam[$i]}')");
        if (!$insert) {
            $allSuccess = false;
                ...
                <h3>Tambah Jadwal</h3>
                    <form method="post" action="">
                        <div class="form-group">
                            ...
                            <input type="text" name="id_jadwalk" value="<?=  $hasilkode ?>" class="form-control" readonly>
                        </div>
                        <div class="form-group">
                            <label>Kelas</label>
                            <select name="id_kelas" class="form-control">
                                ...
                                while ($g = mysqli_fetch_array($query)) {
                                    echo "<option value=' {$g['id_kelas']} '>{$g['nm_kelas']}</option>";
                                }
                                ?>
                            </select>
                        </div>
